...
pub fn chatbox(
  ...
  if *is_chatbox_open {
    // draw frame
    draw_rect(Srgba { red: 0.025, green: 0.0, blue: 0.05, alpha: 0.45 }, position, size, z, window, commands);
  }
  if timer.elapsed().as_secs_f32() < chat_remain_displayed_time && !*is_chatbox_open {
    draw_rect(Srgba { red: 0.025, green: 0.0, blue: 0.05, alpha: 0.2 }, Vector2 { x: position.x, y: position.y + size.y - 9.0 * vh }, Vector2 { x: size.x, y: -16.0 * vh }, z, window, commands);
  }
  if *is_chatbox_open {
...
pub fn load_game_object_animations(asset_server: AssetServer) -> HashMap<GameObjectType, AnimationState>  {
  let game_object_animations: HashMap<GameObjectType, AnimationState> = HashMap::from([
    ...
    (GameObjectType::RedSpawn,                         AnimationState::new(vec![asset_server.load("gameobjects/spawnpoint_red.png")],Vec2 {x: 200.0, y: 200.0},1.0, 0)),
    (GameObjectType::BlueSpawn,                        AnimationState::new(vec![asset_server.load("gameobjects/spawnpoint_blue.png")],Vec2 {x: 200.0, y: 200.0},1.0, 0)),
    (GameObjectType::ControlPoint,                     AnimationState::new(vec![asset_server.load("gameobjects/pointtemp.png")],Vec2 {x: 200.0, y: 200.0},1.0, 0)),

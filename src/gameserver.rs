  ...
  loop {
    ...
    for p_index in 0..players.len() {
      ...
      if players[p_index].is_dead {
        if gamemode_params.do_respawns {
          if players[p_index].death_timer_start.elapsed().as_secs_f32() > gamemode_params.respawn_timer {
            players[p_index].is_dead = false;
          }
        }
